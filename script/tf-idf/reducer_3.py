...
for line in sys.stdin:
    ...
    if current_word != word:
        if current_word:
            corpus_word_count_dict[current_word] = corpus_word_count
            corpus_word_count = 0
        current_word = word
    try:
        corpus_word_count += int(count)
        if word in word_stats_dict.keys():
            word_stats_dict[word].append(word_stats[DOC_NAME_IDX] + ":"
                                         + word_stats[WORD_COUNT_IDX] + ":"
...
for word in corpus_word_count_dict.keys():
    ...
    for word_stats in word_stats_list:
        ...
        try:
            ...
            print(f'{word}:{word_stats[DOC_NAME_IDX]}\t{tf_idf}')
